...
if __name__ == "__main__":
    ...
    #--Two models to run: just prior, surv data only
    filenames = ["prior", "survdata"]
    options   = [(None, None, None), (cut_noisy_hosps, None, None)]
    ...
    #--fit bootsreapped chimeric models
    filenames = ["survplus_hj_boot"]
    options   = [(cut_noisy_hosps, noisy_human_predictions, True)]
    for f,o in zip(filenames,options) :
        d, h, o = o #--these are the options that control what information gets to the model

        #--run through bootstraps
        for boot, h in all_bootstraps.groupby(["bootstrap_sample"]):
            h = h.to_numpy()[:,:-1]
            ...
            peaks_and_times = pd.DataFrame({"peaks":peaks,"times":times, "boot":[boot]*len(times)})
            peaks_and_times.to_csv("./boot/{:s}__boot_{:03d}__peaks_and_times.csv".format(f,boot),index=False)

            #--record quantiles from forecast to use in top row of plot (plot.py)
            quantiles_for_incident_hosps = forecast.compute_quantiles()
            quantiles_for_incident_hosps["t"] = np.arange(0,surv_data.total_window_of_observation)

            quantiles_for_incident_hosps["boot"] = boot
            
            quantiles_for_incident_hosps.to_csv("./boot/{:s}__boot_{:03d}__quantiles.csv".format(f,boot), index=False)

            pickle.dump( forecast, open("./boot/{:s}__boot_{:03d}__forecast.pkl".format(f,boot), "wb"))
