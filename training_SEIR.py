...
if __name__ == "__main__":
    ...
    def model(training_data,ttl, ps):
        ...
        kappa = numpyro.sample("kappa", dist.Gamma( 1/7., 1. ) )
        phi   = 0.025#numpyro.sample("phi", dist.Beta( 20*(0.10), 20*(1 - 0.80) ) )

        r0  = numpyro.sample("R0", dist.Gamma( 1., 1. ) )
        ...
        final, states = jax.lax.scan( lambda x,y: evolve(x,y, (sigma, beta, gamma,kappa) ), jnp.vstack( (S0,E0,I0,H0,R0, H0) ), jnp.arange(T))   

        #--sim
        states = numpyro.deterministic("states",states)
        
        inc_hosps = states[:,-1]*ttl
        inc_hosps = numpyro.deterministic("inc_hosps",inc_hosps)
        
        sim = numpyro.sample("sim_inc_hosps", dist.NegativeBinomial2(inc_hosps, 2.5), obs = training_data)

    nuts_kernel = NUTS(model)
    mcmc        = MCMC( nuts_kernel , num_warmup=1500, num_samples=2000,progress_bar=True)


    mcmc.run(rng_key, extra_fields=('potential_energy',), training_data = training_data, ttl = N, ps = ps)
    mcmc.print_summary()
    ...
    plt.plot(predicted_inc_hosps, color= "red")
