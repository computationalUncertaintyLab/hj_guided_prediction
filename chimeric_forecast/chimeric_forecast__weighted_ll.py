...
class chimeric_forecast(object):
    ...
    def model_specificiation(self
        ...
        #--weight for surv vs hj
        if self.surveillance_data is not None:
            S = len(self.surveillance_data)
        else:
            S=0
        if self.humanjudgment_data is not None:
            H = self.humanjudgment_data.shape[0] #--Number of predictions collected from the crowd of humans
        else:
            H=0

        TTL = float(S+H)
        if TTL==0: #--this handles the case where you want to investigate the prior (ie no data)
            a = numpyro.sample("a", dist.Beta(1.,1.))
        else:
            a = numpyro.sample("a", dist.Beta(TTL*1.,TTL*1.))
        ...
        if self.surveillance_data is None:
            ...
            #--add in a weight that is a*(1/S)

            weights_surv = TTL*a*(1./S)*jnp.ones(S,)
            with numpyro.handlers.mask(mask=mask[:T]), numpyro.handlers.scale(scale=weights_surv) :
        ...
        if self.humanjudgment_data is None:
            ...
            if self.peak_time_and_values:
                ...
                #--Loglikelihood
                weights_hj = TTL*(1-a)*(1./N)*jnp.ones(N,)
                
                with numpyro.plate("observations", N), numpyro.handlers.scale(scale=weights_hj):
